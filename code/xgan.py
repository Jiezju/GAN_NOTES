...
class DecoderPart(layers.Layer):
    ...
    def __init__(self):
        ...
        self.deconv5 = layers.Conv2DTranspose(3, (1, 1), strides=(1, 1), padding='same', use_bias=False,
                                              activation='tanh')
...
class XGAN(tf.keras.Model):
    ...
    def domain_adversarial_loss(self, cartoon_from_cartoon_dataset, real_from_real_dataset):
        ...
        domain_cartoon = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=cdann_cartoon, labels=tf.zeros_like(cdann_cartoon)))
        domain_real = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=cdann_real, labels=tf.ones_like(cdann_real)))

        return domain_cartoon + domain_real

    def generator_loss(self, img_cartoon_dataset, img_reals_dataset, generator_result_from_real,
                       generator_result_from_cartoon):
        # 自监督学习损失
        autoencoder_loss = self.autoencoder_loss(img_cartoon_dataset, img_reals_dataset,
                                                 generator_result_from_cartoon['image_a'],
                                                 generator_result_from_real['image_b'])
        semantic_loss = self.semantic_consistency_feedback_loss(generator_result_from_cartoon,
                                                                generator_result_from_real)
        domain_adversarial_loss = self.domain_adversarial_loss(generator_result_from_cartoon,
                                                               generator_result_from_real)
    ...
    def train_step(self, dataset):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape:
            generator_result_from_real = self.generator(img_reals_dataset,
                                                        is_cartoon=False)  # image_a (carton from real), image_b(real from real)
            generator_result_from_cartoon = self.generator(img_cartoon_dataset,
                                                           is_cartoon=True)  # image_a (carton from cartoon), image_b(real from carton)

            # Discriminator
            disc_loss = self.discriminator_loss(generator_result_from_real, img_cartoon_dataset)

            # Generator
            gen_loss = self.generator_loss(img_cartoon_dataset, img_reals_dataset, generator_result_from_real,
                                           generator_result_from_cartoon)
            ...
            with self.train_summary_writer.as_default():
                ...
                tf.summary.scalar('domain_adversarial_loss', self.domain_adversarial_loss_metric.result(),
