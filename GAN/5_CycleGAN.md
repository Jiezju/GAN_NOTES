...
- **XGAN风格转换**	
	...
	- 问题背景
        ...
        相比于 CycleGAN 和 starGAN 的最终结果的循环一致性，XGAN 考虑的是中间特征层（高层语义）的循环一致性
    ...
    - XGAN 思想
		...
		- 基本网络结构
            ...
            - 每个域对应着编码器 $e$ 和 解码器 $d$
				...
				- $e_1$ 和 $d_1$ 表示 $D_1$ 域的编码器和解码器
				- 编码器 $e_1$ 和 $e_2$ 最后几层网络共享，保证提取的语义特征关联
				- 编码器 $d_1$ 和 $d_2$ 前几层网络共享，保证提取的语义特征关联
			- 一个判别器 $C_e$

				- 目的用于判定中间特征层来自哪个域
				- $e$ 的目标是让 $C_e$ 无法判定来自哪个域，与编码器进行对抗（目的就是让编码器将不同域的图像编码的高层语义特征尽可能相似）

			- 语义循环一致性

				编码器将不同域编码的高层语义特征骗过判别器，但是不同编码器获得高层语义又与编码源图像具有语义相关性
                
                以输入图像 $x$ 为例，对于如下方式的复原图像应该具有语义关联性[图像经过$e_1$ 编码后在经过$d_2$ 解码复原后在进行 $e_2$ 编码]：
     ...
     - 损失函数设计
		...
		- 图像重构损失 $L_{rec}$
			...
			$$L_{rec,1} = E_{x-PD_1}(||x-d_1(e_1(x))||_2)$$   
       ...
       - 编码器对抗损失
            ...
            $$min_{\theta_{e1}, \theta_{e2}} max_{\theta_{dann}} L_{dann}$$
            
            $$L_{dann} = E_{pD_1}\phi(1, C_e(e_1(x))) + E_{pD_2}\phi(1, C_e(e_2(x)))$$
            
            - $\phi$ 表示对抗损失函数，最大化判别器，提高判别精度，最小化生成器损失，使得生成器尽可能逼真

		- 循环一致性损失

			$$L_{sem} = E_{x-PD_1} |e_1(x) - e_2(g_{1-2}(x))|$$
            
            它表示decoder在对两个域进行编码后，在语义特征这个层次上的信息应该一致，从而使得得到的编码具有固定的信息，而且转换后的图像基本特征上和输入是相似的。
            
        - 指导损失

			$$L_{teach} = E_{x-PD_1} |T(x) - e(x)|$$
            
            这是一个可选的项，允许使用先验知识来加速模型的训练，可以看作是对习得的Shared Embedding的一种正则化方式，相当于将 编码信息限制在更有意义的子区域
